...
format:
  revealjs:
    footer: Single Cell RNA-seq Workshop · Lecture 08
    transition: fade
...
-   Previous: [Lec 07 — Functional Analysis](Lecture_07_FunctionalAnalysis.html)
-   **You are here:** Lec 08 — *the compositional complement to differential expression*
-   Next: Lec 08 closes the **core analysis track (00–08)**. What follows (Lec 09–17) are self-paced **bonus tracks** — HPC/Talapas, Cell Ranger, and advanced topics — which you can take in any order. A natural first step is [Lec 15 — Bulk + scATAC-seq](Lecture_15_ATACseq.html), the chromatin counterpart to this expression-based track.
-   **Companion tutorial:** [Tutorial 08 — Differential Abundance with miloR](../Exercise_Folder/Tutorial_08_DifferentialAbundance.html)
-   **Companion book chapter:** [Chapter 8 — Differential Abundance with miloR](../Resources_Folder/Chapter_08_DifferentialAbundance.html)
...
::: incremental
1.  **Running on the unintegrated kNN graph.** If batch effect dominates the embedding, neighbourhoods cluster by sample, and DA falsely "detects" composition shifts that are really batch artifacts. Always run miloR on the **integrated** reduction. In Tutorial 08, the Harmony reduction from Tutorial 05 is stored under the name `"HARMONY"` (all caps), so the call is:

    ``` r
    milo <- buildGraph(milo, k = 30, d = 20, reduced.dim = "HARMONY")
    ```

    If you used the Seurat-anchor path instead, `RunPCA` is run *on the integrated assay*, so the integrated reduction is `"PCA"` — pass `reduced.dim = "PCA"` (`as.SingleCellExperiment` upper-cases reduction names). The trap is with Harmony: there `"PCA"` is the *uncorrected* reduction and would give the unintegrated graph, so you must use `"HARMONY"`.
...
-   **Lec 15** — [Bulk + scATAC-seq](Lecture_15_ATACseq.html) — the chromatin counterpart, and the first of the self-paced bonus tracks
-   **Hands-on:** [Tutorial 08 — Differential Abundance](../Exercise_Folder/Tutorial_08_DifferentialAbundance.html)
