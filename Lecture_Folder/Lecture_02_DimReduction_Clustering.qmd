...
format:
  revealjs:
    footer: Single Cell RNA-seq Workshop · Lecture 02 of 13
    transition: fade
...
-   **You are here:** Lec 02 — *PCA, neighbour graph, Leiden, t-SNE/UMAP on a single sample*
...
::: callout-note
**Default vs. Leiden in practice.** Seurat's `FindClusters()` defaults to **Louvain** (`algorithm = 1`); Leiden (`algorithm = 4`, shown above) is the modern preferred method but needs the `leidenalg` Python package via `reticulate`. For portability, the **workshop tutorials run the Louvain default** — switching to Leiden is the one-line `algorithm = 4` change once that dependency is installed. (`scanpy`'s `sc.tl.leiden` uses Leiden out of the box.)
:::
...
## Leiden clustering — step by step (dup1)
...
## Leiden clustering — step by step (dup2)
...
-   Build the **SNN graph** on the PCs, then cluster with **Leiden** — graph-based methods beat K-means for scRNA-seq
