...
format:
  revealjs:
    footer: Single Cell RNA-seq Workshop · Lecture 03
    transition: fade
...
-   Previous: [Lec 02 — Dim Reduction & Clustering](Lecture_02_DimReduction_Clustering.html)
-   **You are here:** Lec 03 — *finding markers and turning clusters into named cell types by hand*
-   Next: [Lec 04 — Reference-based Annotation](Lecture_04_Reference_Annotation.html)
-   Multi-sample integration follows in [Lec 05 — Multi-Sample Integration](Lecture_05_Integration.html)
-   **Companion tutorial:** [Tutorial 03 — Markers & Manual Annotation](../Exercise_Folder/Tutorial_03_Markers_Annotation.html)
-   **Companion book chapter:** [Chapter 3 — Markers & Manual Cell-Type Annotation](../Resources_Folder/Chapter_03_Markers_Annotation.html)
...
::: callout-note
**Which do you reach for first?** For **well-referenced model organisms (human, mouse)**, reference-based annotation (Lec 04 — Azimuth; other tools include SingleR and CellTypist) is increasingly the practical *starting* point, with manual markers used to confirm and to catch novel states. For **non-model systems** — where no curated reference exists — manual, marker-driven annotation remains essential and is often the *only* option. This workshop teaches both so you can choose based on your system; the two lectures are a connected pair, not alternatives.
:::
...
::: callout-important
**Interferon-stimulated genes (ISGs) on un-integrated data.** In the ifnb dataset, the STIM condition activates dozens of ISGs (`IFIT1`, `ISG15`, `MX1`, `IFI6`, …). When you run `FindAllMarkers` on *un-integrated* data (Tutorial 02 / Tutorial 03), ISGs will appear at the **top of the marker list for multiple clusters** — not because those clusters are distinct cell types, but because every STIM cell expresses them. Using `FindConservedMarkers(grouping.var = "stim")` filters these out; so does using integrated data (Tutorial 05 onward). If your top hit is `ISG15` or `MX1`, that is a condition signature, not a cell-type marker.
:::
...
nc <- 80
cells <- tibble(
  cell_cluster = factor(rep(clusters, each = nc), levels = clusters),
  cell_id      = paste0(rep(clusters, each = nc), "_", rep(1:nc, length(clusters)))
)

dat <- expand_grid(top3, cells) |>
  rowwise() |>
  mutate(expr = ifelse(cluster == cell_cluster,
...
::: callout-warning
**On un-integrated data you will see more clusters than named cell types.** In Tutorial 03 (which follows Tutorial 02's un-integrated clustering), a resolution of ≈ 0.5 on the ifnb dataset typically yields **~15–20 clusters** even though the data contain roughly **13 annotatable PBMC cell types**. The excess clusters arise because each cell type appears as a CTRL version and a STIM version — two islands in the UMAP that get separate cluster IDs. Attempting to find conserved markers for all of them and label every one can trigger downstream errors (too few cells in a condition slice). The practical solutions:
...
-   [CellMarker 2.0](http://bio-bigdata.hrbmu.edu.cn/CellMarker/) — curated, searchable by tissue/disease
...
-   **Lec 04** — [Reference-based Annotation](Lecture_04_Reference_Annotation.html): the automated complement to today's manual workflow
