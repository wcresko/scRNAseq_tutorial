---
title: "L07 — Functional Analysis: Turning DE Lists into Biology"
author: "Bill Cresko"
format:
  revealjs:
...
![](images/mechanisms_overview.png){alt="Mechanisms overview — raster reference" fig-align="center" width="92%"}
...
ggplot(grid, aes(celltype, term, size = count, color = log_p)) +
  ...
  labs(title = "STIM-vs-CTRL GO BP enrichment across cell types",
       x = NULL, y = NULL) +
  theme_minimal(base_size = 11) +
...
2.  **Treating all enriched terms as biology.** 
...
3.  **No multiple-testing correction across databases.** 
...
4.  **Reporting raw p-values.** 
  - Always report `p.adjust` (BH) or `q-value`.
5.  **Ignoring direction in ORA.** 
