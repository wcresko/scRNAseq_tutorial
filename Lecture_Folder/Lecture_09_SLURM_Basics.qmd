...
-   The green `SSH: login.talapas.uoregon.edu` banner in the status bar confirms you're remote. Verify with:
:::
...
When VS Code is connected via Remote-SSH, its integrated terminal opens a shell **on the Talapas login node**, not on your laptop. You edit files in the VS Code editor (left) while running commands in the terminal (bottom) — both operating on the same remote files.
:::
...
-   This is the core workflow for scRNA-seq analysis on Talapas:
:::
...
sbatch run_rscript.sbatch 01_qc_preprocessing.R  # or submit as a batch job
tail -f logs/seurat_qc_*.out            # watch the job as it runs
```
