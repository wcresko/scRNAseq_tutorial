...
-   Use the `run_rscript.sbatch` wrapper and `run_pipeline.sbatch`
-   Know why you **pre-cache `ifnb`** and what `OBJ_DIR` is for
...
| Script | Reads | Writes |
|---|---|---|
| `01_qc_preprocessing` | `Kang18_8vs8()` | `ifnb_preprocessed.rds` |
...
## `run_pipeline.sbatch` — the whole series at once
...
-   Submit once: `sbatch run_pipeline.sbatch`, then `seff <jobid>` to confirm `COMPLETED`.
...
## `OBJ_DIR` — where objects land

::: incremental
-   By default the scripts write objects to `../objects/`.
-   Set `OBJ_DIR` to redirect them — e.g. to fast, roomy `/scratch`:
:::
...
-   `run_rscript.sbatch` runs any one step; `run_pipeline.sbatch` runs all eight.
-   **Pre-cache `ifnb`** on a login node; use `OBJ_DIR` to relocate outputs.
