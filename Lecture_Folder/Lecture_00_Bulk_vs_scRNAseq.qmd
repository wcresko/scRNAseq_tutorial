...
-   **Companion tutorial:** [Tutorial 00 — Setup: R, RStudio, Quarto & VS Code/Talapas](../Exercise_Folder/Tutorial_00_Setup_RStudio_Packages.html)
...
::: callout-note
**Alternative to the sparsity panel above.** Original figure: **Yu *et al.* 2020** (review; on Hope Healey's Lecture 1, slides ~28–36).
:::
...
::: callout-note
**Visual for the 3′ UTR caveat above.** Original concept figure: **Healey *et al.* 2022** (on Hope Healey's Lecture 1, slides ~25–26) — the source to embed if you prefer her version.
:::
...
::: callout-note
**Alternative to the library-prep slides above**, for side-by-side comparison. Original figures: **10x Genomics** Chromium Single Cell documentation — [10xgenomics.com/support/single-cell-gene-expression](https://www.10xgenomics.com/support/single-cell-gene-expression) (on Hope Healey's Lecture 1, slides ~17–22).
:::
...
::: callout-note
**Alternative to the barcodes + UMIs slide above.** Original figure: **10x Genomics** 3′ reagent kit documentation — [10xgenomics.com/support/single-cell-gene-expression](https://www.10xgenomics.com/support/single-cell-gene-expression) (on Hope Healey's Lecture 1, slide ~23).
:::
...
::: callout-note
**Visual for the "under the hood" slide above.** Original figures: **10x Genomics** Cell Ranger algorithms overview — [10xgenomics.com/support/software/cell-ranger/latest/algorithms-overview](https://www.10xgenomics.com/support/software/cell-ranger/latest/algorithms-overview) (on Hope Healey's Lecture 1, slides ~54–69).
:::
...
-   **On the cluster — VS Code + Quarto on Talapas.** On Friday we run the *same* `.qmd`/`.R` material on UO's **Talapas** HPC cluster. You'll edit and run it through **VS Code** connected to Talapas over **Remote-SSH**, with the **Quarto extension** to render and run notebooks right on the cluster.

## VS Code + Quarto + Talapas (interactive mode)

::: incremental
-   **VS Code** is a free editor that can open a folder *on Talapas* over SSH — you edit and run remote files as if they were local.
-   Install the **Remote – SSH** extension, add the host `login.talapas.uoregon.edu`, and connect with your Duck ID + Duo.
-   Install the **Quarto** extension so you can render and run `.qmd` notebooks (and the workshop's `.R` scripts) inside VS Code on Talapas.
-   Today we only use Talapas in **interactive mode** (an `srun`/`salloc` session — a shell on a compute node). Submitting **batch** jobs with SLURM (`sbatch`) is the Friday-morning track — **Modules 09–10**.
:::

➡️ Full walkthrough: [**VS Code & Talapas**](../Resources_Folder/Chapter_Day0_1_Computer_Systems.html) · hands-on in [Tutorial 00](../Exercise_Folder/Tutorial_00_Setup_RStudio_Packages.html).
...
-   The laptop handles `ifnb` (~14k cells) fine; **real** datasets (hundreds of thousands of cells, or raw FASTQs through Cell Ranger) need the cluster's CPUs, RAM, and storage.
...
-   **Tools:** run the notebooks in **RStudio** on your laptop; on Friday, the same material on **Talapas** via **VS Code + Quarto** (interactive now, SLURM batch in Modules 09–10)
:::

## Coming up next

-   **Get set up first:** [Tutorial 00 — R, RStudio, Quarto & VS Code/Talapas](../Exercise_Folder/Tutorial_00_Setup_RStudio_Packages.html) (and [Software Setup](../Software_Setup.qmd))
