...
|-------------------------|-----------------------|---------------------------------------------------------------------------|
| `/home/<duckid>`        | ~20 GB (small)        | Personal files, scripts, dotfiles; small software installs — **not** large data or references |
...
-   Specify version numbers for reproducibility (e.g., `R/4.4.1`, not just `R`).
:::
:::
...
The files that implement this pattern in a fully reproducible project are an `renv.lock` (Layer 1), a `Dockerfile` (Layer 2), and a CI workflow such as `.github/workflows/build-image.yml` that rebuilds the image on a schedule. For the data these environments are built around, see [Datasets](../Datasets.html).
