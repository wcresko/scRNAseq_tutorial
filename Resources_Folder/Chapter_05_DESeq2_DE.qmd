...
The choice of NB rather than Poisson is non-negotiable for RNA-seq counts. Poisson assumes variance = mean; real RNA-seq has variance > mean (over-dispersion), and Poisson testing is **anti-conservative** — it produces falsely-significant p-values. The NB adds a per-gene parameter to absorb the extra variance; results are honest. See [Appendix D § 1–2](Appendix_D_Statistical_Foundations.html) for the underlying distributions.
...
This is a **Bayesian-flavored procedure** — the trend is the prior, the per-gene MLE is the likelihood, the shrunk value is the posterior. See [Appendix D § 7](Appendix_D_Statistical_Foundations.html) for the connection.
...
The shrinkage is a Bayesian procedure with `apeglm`'s prior — see [Appendix D § 7](Appendix_D_Statistical_Foundations.html) for the connection to the dispersion shrinkage in §5.4. It's the same idea: borrow strength from all genes to stabilize each gene's estimate.
...
- **Statistical foundations:** [Appendix D §§ 1–8](Appendix_D_Statistical_Foundations.html)
