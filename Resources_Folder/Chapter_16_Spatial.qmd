...
3. **Normalize** with `SCTransform` — preferred over `LogNormalize` for spatial because each spot captures the transcriptome of ~1–10 cells (tissue-dependent) rather than one. This wider dynamic range means count distributions are further from the Poisson assumption that `LogNormalize` implicitly relies on; `SCTransform`'s regularized negative binomial model adjusts per-spot dispersion and handles outlier spots more robustly.
...
@tbl-ch16-errors collects the errors most commonly encountered in a Seurat Visium workflow, their causes, and the fix.
...
| `SpatialFeaturePlot` shows nothing on the tissue | The `images` slot wasn't loaded | Re-run `Load10X_Spatial()`, or attach the image with `Read10X_Image()` pointing at the Space Ranger `spatial/` folder |
