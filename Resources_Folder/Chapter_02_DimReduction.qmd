...
Both algorithms optimize **modularity** — a measure of how much denser within-community edges are than the random expectation. **Louvain** is Seurat's default and is what the **workshop tutorials run** (`FindClusters(seu, resolution = 0.5)`, i.e. `algorithm = 1`), because it has no extra dependencies. **Leiden** (Traag, Waltman, van Eck 2019) is the modern preferred refinement — it improves on Louvain by guaranteeing well-connected communities — but it needs the `leidenalg` Python package via `reticulate`, so we present it as the one-line upgrade rather than the default.
