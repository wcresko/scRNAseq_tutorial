...
::: {.callout-note .key-concept title="🔑 Key concept — the three-step pipeline is a strict dependency chain"}
...
::: {.callout-note .key-concept title="🔑 Key concept — what a principal component captures"}
...
::: {.callout-note .key-parameter title="⚙️ Key parameter — RunPCA npcs"}
...
The standard tool is the **scree plot** (sometimes called an elbow plot): for each PC, plot the variance it explains, in descending order. The shape is a steep initial drop, an **elbow**, and a long noise floor. @tbl-ch02-pc-selection summarizes the main PC-selection strategies, their cost, and when to reach for each.
...
: PC-selection strategies and when to use each. Leaning high is the safer error: an extra few noise PCs slow runtime minimally but a discarded PC carrying a rare subtype cannot be recovered. {#tbl-ch02-pc-selection}
...
::: {.callout-note .key-parameter title="⚙️ Key parameter — FindNeighbors k.param"}
...
::: {.callout-note .key-parameter title="⚙️ Key parameter — FindClusters resolution"}
...
@tbl-ch02-viz summarizes both methods side by side before the subsections walk the details.
...
::: {.callout-note .key-concept title="🔑 Key concept — what UMAP does and does not preserve"}
...
::: {.callout-note .key-parameter title="⚙️ Key parameter — RunUMAP n.neighbors and min.dist"}
...
When you print PCA results with `print(seu[["pca"]], dims = 1:5, nfeatures = 5)`, you see the top 5 positive and negative gene loadings for the first five PCs. These are the genes pushing cells to the high or low end of each PC axis. @tbl-ch02-loadings shows the three canonical patterns and their interpretation. What to look for:
...
: Canonical PC loading patterns and their interpretation. Reading the top 5 positive and negative loadings per PC is the fastest sanity check after `RunPCA`. {#tbl-ch02-loadings}
...
@tbl-ch02-errors collects the errors most frequently encountered during dimensionality reduction and clustering, together with their root cause and the fix.
...
: Common errors in dimensionality reduction and clustering, their root cause, and the fix. {#tbl-ch02-errors}
