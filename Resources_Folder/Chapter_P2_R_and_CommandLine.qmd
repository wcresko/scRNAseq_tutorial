...
Most modern R for biology is written in **tidyverse** style [@wickham2019tidyverse] — a coherent family of packages (`dplyr`, `tidyr`, `ggplot2`, `readr`, …) that share a consistent, pipe-friendly grammar. Six `dplyr` verbs cover \~80% of what you'll do with data frames:
...
::: {.callout-tip title="Skills used in later modules — prerequisite check"}
**R and command-line skills introduced here:**

| Skill covered here | First used in |
|---|---|
| Shell navigation: `pwd`, `ls`, `cd`, absolute vs relative paths | [Tutorial P2 (hands-on)](../Exercise_Folder/Tutorial_P2_R_and_CommandLine.html) · [Module 09](../Exercise_Folder/Tutorial_09_SLURM_Basics.html) §5–6 (navigating to `scripts/` before `sbatch`) |
| Pipes `\|` and redirection `>` / `>>` | [Module 09](../Exercise_Folder/Tutorial_09_SLURM_Basics.html) §6 (`echo` into `~/.bash_profile`); reading job logs with `cat logs/*.out` |
...
| `install.packages()` once vs `library()` every session | [Tutorial 00](../Exercise_Folder/Tutorial_00_Setup_RStudio_Packages.html) §1; top of every tutorial |
:::
...
The canonical, up-to-date package install block (CRAN + Bioconductor + GitHub packages for Modules P1–10) lives in **[Tutorial 00 — Get Up and Running](../Exercise_Folder/Tutorial_00_Setup_RStudio_Packages.html)** §2 and in **[Tutorial P2](../Exercise_Folder/Tutorial_P2_R_and_CommandLine.html)** Part C. Both blocks are identical. Do **not** install from memory — copy the block from one of those two pages so you get the correct `setdiff()` guard and the exact package versions the workshop expects.
:::

The foundational citations for the two tools this chapter covers are the R language itself [@rcoreteam] and the tidyverse [@wickham2019tidyverse].
