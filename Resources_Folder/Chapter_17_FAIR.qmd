---
title: "Chapter 17 — FAIR Principles & Data Sharing"
...
::: callout-note
**Where this chapter sits.** Companion to **[Lecture 17](../Lecture_Folder/Lecture_17_FAIR_DataSharing.html)** and **[Tutorial 17](../Exercise_Folder/Tutorial_17_FAIR_Metadata.html)**. **Cross-reference:** [Appendix E — Gene-Expression Databases & File Formats](Appendix_E_Databases_FileFormats.html), [Appendix G — Methods Section Template](Appendix_G_Methods_Template.html).
:::
...
Wilkinson et al. [@wilkinson2016fair] proposed four guiding principles for data management, summarized in @tbl-ch17-fair.
...
: The four FAIR principles [@wilkinson2016fair]. FAIR is not synonymous with "open" — FAIR data can be access-controlled while still satisfying all four criteria. {#tbl-ch17-fair}
...
- **GEO** (`GSE######`) [@barrett2013geo] — the **processed** data: counts matrices, normalized files, expression metadata. GEO records pull SRA + BioSample IDs in.

For scRNA-seq, you create all four. The [Appendix E](Appendix_E_Databases_FileFormats.html) reference card details what each one holds.

### Single-cell-aware repositories

Beyond the generic raw archive, deposit in a scRNA-seq–native repository so the data is *queryable as scRNA-seq*. @tbl-ch17-repos summarizes the major options.

| Repository | Run by | What it accepts | Strengths |
|---|---|---|---|
| **CELLxGENE Discover** [@czi2025cellxgene] | CZI | AnnData (`.h5ad`) with required metadata | Browseable atlas, in-browser visualization, schema-validated |
| **Human Cell Atlas Data Portal** [@regev2017hca] | HCA Consortium | Raw + processed, full provenance graph | Strict QC, integrated atlas |
...
: Single-cell-aware repositories for scRNA-seq data deposition. The recommended workflow is triple-deposit: raw FASTQs to SRA, processed data to GEO, and AnnData to CELLxGENE. {#tbl-ch17-repos}
...
The CELLxGENE schema [@czi2025cellxgene] is the most demanding of the major repositories and also the most reusable. Its insistence on ontology-typed fields is exactly what makes cross-dataset atlases possible [@osumisutherland2021cell]. The required fields are listed in @tbl-ch17-cellxgene; meeting this bar gets you GEO + HCA compatibility for free.
...
: Required CELLxGENE schema fields [@czi2025cellxgene] and their controlled vocabularies. Ontology IDs are looked up at the [Ontology Lookup Service](https://www.ebi.ac.uk/ols4/). The mapping from "B cell" to `CL:0000236` is what makes your dataset interoperable with every other CELLxGENE-deposited dataset. {#tbl-ch17-cellxgene}

## 17.5 File formats and what to deposit

@tbl-ch17-formats lists the file types you should deposit at each layer of the data lifecycle.
...
: Recommended file formats for each layer of an scRNA-seq data deposition. AnnData (`.h5ad`) is the preferred archival format for annotated objects; `.rds` files are Seurat-version-locked and are not future-proof. {#tbl-ch17-formats}
...
::: {.callout-note .key-parameter title="⚙️ Key parameter — CELLxGENE schema / required fields and controlled vocabulary strictness"}
...
**Do this instead:** at experimental design time, build the **canonical sample sheet** ([Tutorial 17](../Exercise_Folder/Tutorial_17_FAIR_Metadata.html) ships a template). Include every field you'll need at deposition. Version-control it alongside your code. As samples come in, update the sheet. At the end of the project, the sheet *is* your BioSample / SRA / GEO upload.
...
[Tutorial 17](../Exercise_Folder/Tutorial_17_FAIR_Metadata.html) walks all of this hands-on, generating the BioSample / SRA / GEO upload `.tsv` files and the AnnData from the workshop's `ifnb_annotated_final.rds`.

## 17.8 Common errors

@tbl-ch17-errors collects the errors most commonly encountered during FAIR metadata preparation and submission, their causes, and the fix.
...
: Common FAIR data-deposition errors, their underlying causes, and fixes. {#tbl-ch17-errors}

## 17.9 Where this material is also discussed

- **Lecture:** [Lecture 17](../Lecture_Folder/Lecture_17_FAIR_DataSharing.html)
- **Tutorial:** [Tutorial 17](../Exercise_Folder/Tutorial_17_FAIR_Metadata.html)
- **Database & file-format reference:** [Appendix E](Appendix_E_Databases_FileFormats.html)
- **Methods-section template (the "Data availability" paragraph):** [Appendix G](Appendix_G_Methods_Template.html)
- **scNotebooks Module 13** — parallel walkthrough

## 17.10 Going further

The principles come from the FAIR paper [@wilkinson2016fair]; the single-cell infrastructure that operationalizes them is the Human Cell Atlas [@regev2017hca], its cell-type ontology standards [@osumisutherland2021cell], and the CZ CELLxGENE Discover platform [@czi2025cellxgene] (whose schema is documented at [github.com/chanzuckerberg/single-cell-curation](https://github.com/chanzuckerberg/single-cell-curation)). The canonical raw/processed archive is NCBI GEO/SRA [@barrett2013geo], and large reference atlases such as Tabula Sapiens [@tabulasapiens2022] show what well-shared data enables. Also consult the **NIH Data Management & Sharing Policy** (2023), required for all NIH-funded studies. The full curated list is on [Key Papers, Reviews & Benchmarks](../Key_Papers.html).
