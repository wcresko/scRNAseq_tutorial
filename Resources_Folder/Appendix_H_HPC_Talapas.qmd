---
title: "Appendix H - High-Performance Computing with Talapas"
author: "Bill Cresko"
format:
  html:
    toc: true
...
# Appendix H: High-Performance Computing with Talapas

## Overview

::: {.callout-tip title="Pre-requisite Material"}
This appendix covers the University of Oregon's high-performance computing (HPC) cluster, Talapas. You will learn how to access the cluster, submit jobs using SLURM, manage software modules, and run GPU-accelerated computations. See Appendix E for background on cluster computing concepts.
:::
...
-   Over 14,500 CPU cores
-   129 TB of memory
-   ~89 GPUs (NVIDIA A40 through H100)
...
These are the current Talapas partitions (matching the [SLURM tutorial](../Exercise_Folder/Talapas/Tutorial_Talapas_01_SLURM_Basics.html) and [VS Code & Talapas](VSCode_Talapas.html) — always confirm against `sinfo` and the [Talapas Knowledge Base](https://uoracs.github.io/talapas2-knowledge-base/)):

-   `compute` --- standard CPU jobs (up to 1 day)
-   `computelong` --- long CPU jobs (up to 14 days)
-   `gpu` / `gpulong` --- GPU jobs (1 day / 14 days)
-   `memory` / `memorylong` --- high-memory CPU jobs, up to 4 TB RAM (1 day / 14 days)
-   `interactive` / `interactivegpu` --- short interactive sessions (12 h / 8 h)
-   `preempt` --- access to all nodes; jobs may be preempted
