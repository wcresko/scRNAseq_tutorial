...
The hands-on [SLURM Basics tutorial](../Exercise_Folder/Tutorial_09_SLURM_Basics.html) walks through submitting and monitoring batch and interactive jobs step by step — including the `sbatch`/`run_rscript.sbatch` wrapper, the `logs/` directory, `seff` for resource checking, and the end-to-end R-script pipeline covered in [Module 10](../Exercise_Folder/Tutorial_10_Talapas_Pipeline.html).
:::
...
**Skills used in later modules — prerequisite check:**

| Skill covered here | First used in |
|---|---|
| SSH login, login vs compute nodes, `srun --pty bash` | [Tutorial P1 (hands-on)](../Exercise_Folder/Tutorial_P1_Computer_Systems.html) · [Module 00](../Exercise_Folder/Tutorial_00_Setup_RStudio_Packages.html) §6 |
| `module load R` (and pinning a version) | [Module 09](../Exercise_Folder/Tutorial_09_SLURM_Basics.html) §5.5; required inside every batch script |
| `sbatch` batch scripts, `#SBATCH` directives, `--account`, `--partition` | [Module 09](../Exercise_Folder/Tutorial_09_SLURM_Basics.html) §6 |
| `squeue`, `scancel`, `seff`, `sacct` | [Module 09](../Exercise_Folder/Tutorial_09_SLURM_Basics.html) §7–8 |
| `/projects` vs `/scratch` file systems | Required for locating output in [Module 10](../Exercise_Folder/Tutorial_10_Talapas_Pipeline.html) |
| `renv` + Apptainer reproducible environments | [Module 09–10](../Exercise_Folder/Tutorial_09_SLURM_Basics.html) batch scripts; the workshop container |

**One Friday-specific detail:** [Module 09](../Exercise_Folder/Tutorial_09_SLURM_Basics.html) requires you to create a `logs/` directory before submitting the first batch job (`mkdir -p logs`). SLURM will not create it for you.
:::
