---
title: "Appendix A: Single-Cell Biology Refresher"
...
scRNA-seq measures the transcriptome **one cell at a time**, which is what makes it powerful (you see cell-to-cell heterogeneity) and what makes it hard (each cell yields only a tiny amount of RNA, so the data are noisy and sparse). The first single-cell transcriptome was sequenced in 2009 [@tang2009mrna]; droplet chemistries later scaled the approach to thousands of cells per run [@macosko2015dropseq; @zheng2017chromium].
...
- **Cells are not biological replicates** — cells from the same tissue from the same donor share genetics, library prep, and dissociation. Statistically, the unit is the *donor*, not the cell, and ignoring this produces large numbers of false discoveries [@squair2021confronting]. (This is why pseudobulk DE in Lecture 06 is the right answer for condition contrasts)
...
- **[Single-cell best practices](https://www.sc-best-practices.org/preamble.html)** — the same biology in scRNA-seq context; the companion paper [@heumos2023bestpractices] and the original best-practices tutorial [@luecken2019bestpractices] are the field's standard end-to-end references
- **[10x Genomics support / Chromium](https://www.10xgenomics.com/support/single-cell-gene-expression)** — manufacturer's documentation, very thorough on chemistry [@zheng2017chromium]
...
- [Lecture 00 — Tools and Foundational Concepts](../Lecture_Folder/Lecture_00_Tools_Found_Concepts.html) — picks up exactly where this appendix leaves off
- [Glossary](Glossary.html) — for the *analytical* terms (PCA, UMAP, Leiden, integration, ...)
- [Appendix E — Databases & File Formats](Appendix_E_Databases_FileFormats.html) — for what FASTQ / `.h5` / `.h5ad` / `.rds` actually contain
- [Appendix B — Statistical Foundations](Appendix_B_Statistical_Foundations.html) — for the statistics used in DE / clustering / DA
