...
**Default: 3,000.** This parameter tells Cell Ranger approximately how many cells to expect, which influences the knee-based component of cell calling. It is not a hard cutoff — Cell Ranger adjusts its algorithm around this expectation rather than truncating exactly at it. Providing an accurate value (or a reasonable estimate) is most important when the knee is soft or ambiguous. If you loaded 5,000 cells, set `--expect-cells=5000`; if you loaded 10,000, set it to 10,000. Using the default 3,000 for a high-loading experiment that actually produced 12,000 cells can cause over-conservative cell calling, where Cell Ranger under-calls cells because it did not expect so many. The "Estimated number of cells" in `web_summary.html` is the diagnostic: if it is far below your expected target, re-run with a more accurate `--expect-cells`.
:::
