...
Cell Ranger is the official 10x pipeline, but it is not the only way to turn FASTQs into a counts matrix, and the open-source alternatives are often faster and more flexible. **STARsolo** [@kaminow2021starsolo] reproduces Cell Ranger's logic inside the STAR aligner [@dobin2013star] and is largely concordant with it; **alevin-fry / salmon** [@srivastava2019alevin] and **kallisto | bustools (kb-python)** [@melsted2021kallistobustools] use lightweight (pseudo)alignment for constant-memory, very fast quantification. All three accept custom references easily, which matters for non-model organisms where no 10x reference exists. They differ mainly in how they handle multimapping reads, intronic counts, and EmptyDrops-style cell calling, so a sample quantified two ways will agree on most genes but not every count — worth knowing when you compare results across pipelines.
