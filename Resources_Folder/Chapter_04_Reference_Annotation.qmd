...
::: {.callout-note .key-concept title="🔑 Key concept — what reference-based annotation actually does"}
...
Azimuth wraps Seurat's anchor-based mapping [@hao2021integrated] (the same method as Seurat integration, applied with a query → reference orientation). The key design advantage is that the reference UMAP and cell labels are precomputed once from a large atlas dataset; at query time, your cells are projected *into* that manifold rather than re-computing everything jointly. This is fundamentally different from methods that require both the query and reference to be processed together: because Azimuth's reference is frozen at publication time and its UMAP is precomputed, mapping a new query to it is fast (minutes on a laptop), consistent across queries, and does not require re-running the reference-side integration every time. The shared feature space between query and reference is determined by finding mutual nearest neighbors across the two datasets after projecting both into a common CCA subspace — the same anchor-finding machinery used for multi-sample integration in Chapter 5. Pre-built atlas-quality references are available:
...
::: {.callout-note .key-parameter title="⚙️ Key parameter — Azimuth mapping.score threshold"}
