...
# Out:  ../objects/pbmc_atac_clustered.rds, ../output/Mod9/
# Figures/tables (match the Mod9 notebook filenames): ../output/Mod9/Mod9_C*_*
...
OUT_DIR  <- Sys.getenv("OUT_DIR",  "../output/Mod9") # figures/tables, named to match Tutorial_09.qmd
...
# Table out: first rows of the raw fragment file (Mod9_C2)
frag_preview <- frag.file
colnames(frag_preview) <- c("chrom", "start", "end", "cell_barcode", "duplicate_count")
write_csv(as_tibble(frag_preview), file.path(OUT_DIR, "Mod9_C2_fragment_preview.csv"))
...
# Step 5 — Visualize QC (on the un-filtered object, matching the notebook) (Mod9_C6)
...
p_qc_density <- (a1 | a2) +
  plot_annotation(
    ...
    caption  = "Module 9 · scATAC-seq with Signac")
ggsave(file.path(OUT_DIR, "Mod9_C6_qc_density_scatter.png"), p_qc_density,
...
p_qc_vln <- p_qc_vln +
  plot_annotation(
    ...
    caption  = "Module 9 · scATAC-seq with Signac")
ggsave(file.path(OUT_DIR, "Mod9_C6_qc_violins.png"), p_qc_vln,
...
# Figure out: LSI component vs sequencing-depth correlation (Mod9_C8)
...
ggsave(file.path(OUT_DIR, "Mod9_C8_depthcor.png"), p_depthcor,
...
# Figure out: UMAP coloured by SLM cluster (Mod9_C9)
p_umap <- DimPlot(pbmc, label = TRUE) +
  ...
  labs(title    = "scATAC-seq clusters — PBMC 10k v1",
       ...
       x        = "UMAP 1",
       y        = "UMAP 2")
ggsave(file.path(OUT_DIR, "Mod9_C9_umap_clusters.png"), p_umap,
...
cat("Wrote Mod9 figures/tables to", OUT_DIR, "\n")
