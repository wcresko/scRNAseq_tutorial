...
Each tutorial creates two folders **in the directory you run it from** (the folder the `.qmd` is in): a **`data/`** folder for the chained `.rds` objects passed from one tutorial to the next, and an **`output/ModN/`** folder for that module's figures and tables. So **keep all the tutorial `.qmd` files in one folder** and they will share a single `data/`. R-package datasets (ifnb, airway, stxBrain) are cached by their packages and never land in `data/`; only the manually-downloaded bonus datasets do:

```
your_tutorial_folder/            # wherever you put the .qmd files
├── Tutorial_01 … Tutorial_17 (.qmd)
├── data/                        # created on first run, next to the .qmd
...
Tutorials 01–08 need nothing in `data/` to start — `ifnb` is cached by Bioconductor's `muscData` package on first use. They only *write* to `data/` (the chained `.rds` objects). When you run a setup chunk it prints the absolute path it's writing to, so you can always find your files. (On the Talapas HPC track the layout differs — the numbered scripts run from `Exercise_Folder/scripts/` and read/write `../objects/` and `../data/`.)
...
```bash
# Run this from the folder your Tutorial 13 .qmd is in (creates data/ next to it)
mkdir -p data && cd data
...
```bash
# Run this from the folder your Tutorial 15 .qmd is in (creates data/ next to it)
mkdir -p data && cd data
