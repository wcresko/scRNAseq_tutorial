...
| `airway` | `library(airway); data(airway)` | `readRDS("../data/airway.rds")` |
...
1.  Download the file into your project's `data/` folder:
    ...
    -   **airway** → <a href="rds/airway.rds" download>`airway.rds`</a> — the bulk-RNA-seq `RangedSummarizedExperiment` used in Tutorial 06 Part A
2.  Read it in at the top of the tutorial, in place of the package load:
    ...
    airway <- readRDS("../data/airway.rds")     # Tutorial 06 — instead of library(airway); data(airway)
    ```
:::
...
download.file(file.path(base, "airway.rds"),   "../data/airway.rds",   mode = "wb")
```
...
The `airway` package is small and usually installs cleanly, but if it won't, download <a href="rds/airway.rds" download>`airway.rds`</a> into your project `data/` and read it in **instead of** `library(airway); data(airway)`:

```r
airway <- readRDS("../data/airway.rds")   # identical RangedSummarizedExperiment
```
