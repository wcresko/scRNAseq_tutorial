...
By convention every tutorial reads from `../data/` — a `data/` folder at the repository root. R-package datasets (ifnb, airway, stxBrain) are cached by their packages and never land in `data/`; only the manually-downloaded bonus datasets do:

```
scrnaseq_workshop/
├── Exercise_Folder/
│   └── Tutorial_01 … Tutorial_17 (.qmd) + scripts/
└── data/
...
Tutorials 01–08 need nothing in `data/` to start — `ifnb` is cached by Bioconductor's `muscData` package on first use. They only *write* to `data/` (the chained `.rds` objects passed from one tutorial to the next).
...
|---|---|---|
| `HumanPrimaryCellAtlasData()` | `celldex` | Module 04 (SingleR) |
| Azimuth `pbmcref` | `Azimuth` / `SeuratData` | Module 04 (Azimuth) |
