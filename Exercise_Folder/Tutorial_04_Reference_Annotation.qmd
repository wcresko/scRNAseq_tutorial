...
```{r}
#| label: lst-M4-azimuth
#| lst-cap: "Run Azimuth (pbmcref) to predict hierarchical cell-type labels, then plot level-2 calls on the UMAP."
...
```{r}
#| label: lst-M4-azimuth_confidence
#| lst-cap: "FeaturePlot of per-cell Azimuth mapping scores — cells that project poorly into the reference appear in dark/cool colours."
...
```{r}
#| label: lst-M4-compare
#| lst-cap: "Raw cross-tabulation of author ground-truth labels vs Azimuth level-2 predictions."
...
::: {.callout-tip title="Reading the output"}
The `table()` printout (@lst-M4-compare) is a contingency matrix with ground-truth cell types on rows and Azimuth `predicted.celltype.l2` labels on columns; each cell value is the number of cells with that combination. For a clean PBMC dataset like `ifnb`, most cells should land in one off-diagonal cell per row — you want large numbers concentrated near the "correct" column. Zero rows or columns indicate a label present in one system but not the other. The heatmap below (@lst-M4-compare_heatmap) makes patterns easier to read at a glance.
:::

A cleaner cross-tabulation as a heatmap (@lst-M4-compare_heatmap):

```{r}
#| label: lst-M4-compare_heatmap
#| lst-cap: "Confusion heatmap of author ground-truth labels vs Azimuth level-2 predictions."
...
A simple "agree-or-flag" rule: keep the ground-truth label where Azimuth agrees; otherwise mark for review (@lst-M4-reconcile).
...
```{r}
#| label: lst-M4-reconcile
#| lst-cap: "Harmonize ground-truth and Azimuth labels to a coarse vocabulary, flag disagreements, and tabulate the final calls."
