...
::: {.callout-important title="Where your output goes — read this once"}
The setup chunk below creates two folders **in your current working directory** (the folder the `.qmd` is in):

- **`output/Mod1/`** — every figure (`.png`) and table (`.csv`) this tutorial makes
- **`data/`** — the `.rds` hand-off object that Tutorials 02–08 read back in

When you run the setup chunk it prints the exact absolute path (`This module writes its figures & tables to: …`) — look there if you can't find a file. Two practical rules:

1. **Keep all the tutorial `.qmd` files in one folder.** Tutorial 01 writes `data/ifnb_preprocessed.rds`; Tutorial 02 reads it from the same `data/`. If each `.qmd` lives in a different folder, the hand-off breaks.
2. **Check `getwd()`** if nothing appears — in RStudio, "Run Current Chunk" uses the folder the `.qmd` sits in, but a stray `setwd()` or a different RStudio setting can change that.
:::
