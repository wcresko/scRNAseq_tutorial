...
::: callout-tip
**Want to scale up to a bigger dataset later?** Once you've worked through 01 → 04 on `ifnb`, the parallel **Talapas Advanced series** in `Exercise_Folder/Talapas/` re-do the same workflow on a 20k cell, 250 MB dataset. They are written to be run on **Talapas** as R + SLURM jobs — see [Tutorial — SLURM Basics](Talapas/Tutorial_Talapas_01_SLURM_Basics.html).
:::
...
-   **Distributed via:** the Bioconductor [`muscData`](https://bioconductor.org/packages/muscData/) package as `Kang18_8vs8()` — cached locally through `ExperimentHub` on first download
...
**`library(muscData)` errors with "there is no package called 'muscData'"** — `muscData` is a Bioconductor experiment-data package, not on CRAN. Install with `BiocManager::install("muscData")` (which itself needs `install.packages("BiocManager")` first). The historical `SeuratData::InstallData("ifnb")` route is no longer used here — the Satija lab's data server has been intermittently unavailable — and the `scRNAseq` package dropped its per-dataset accessor functions (e.g. `KangHumanPBMCData()`) in v2.18 in favor of a moving-target `fetchDataset()` API. `muscData::Kang18_8vs8()` bundles the same Kang *et al.* 2017 data with a stable interface.
...
**SoupX errors with "no raw matrix found"** — this is correct behaviour. The `Kang18_8vs8()` download only ships the *filtered* counts. SoupX needs both. The Step 5 callout flags this — the SoupX block is illustrative on `ifnb`. Run it for real on data that has a Cell Ranger `raw_feature_bc_matrix/` folder.
:::
...
# BiocManager::install(c("muscData", "SingleCellExperiment"))
...
::: callout-note
`muscData` is a **Bioconductor** experiment-data package (not on CRAN), so it has to go through `BiocManager::install()`. The first call to `Kang18_8vs8()` downloads \~25 MB through `ExperimentHub` into your local cache; subsequent calls are instant.

Earlier versions of this tutorial used `SeuratData::InstallData("ifnb")`. That route depends on the Satija lab's data server (`seurat.nygenome.org`), which has been intermittently unavailable. A brief detour through `scRNAseq::KangHumanPBMCData()` also broke because `scRNAseq` v2.18 removed its per-dataset accessor functions. `muscData::Kang18_8vs8()` exposes the same Kang *et al.* 2017 PBMCs through a stable interface.
:::
...
# Drop the multiplexed multiplets so we end up with the same set of singletons
# that the historical `ifnb` Seurat object shipped (~14k cells), and drop any
# cells without an author-assigned cell type label.
...
::: callout-warning
**SoupX needs both the raw and filtered Cell Ranger matrices** (`raw_feature_bc_matrix/` and `filtered_feature_bc_matrix/`) because it learns the soup profile from empty droplets. `Kang18_8vs8()` only ships the **filtered** counts, so you cannot run SoupX on `ifnb` directly. The code below is the canonical pattern — use it on **your own** 10x data, or on the [Talapas Advanced series](Talapas/Tutorial_Talapas_03_QC_Preprocessing.html) where the raw matrix is available.
:::
...
| Step 7 — `subset()` QC filter (`nFeature_RNA`, `percent.mt`)    | ~35,635 | ~22,000 |
| Step 8 — `NormalizeData` (no dimension change)                  | ~35,635 | ~22,000 |
...
-   Dataset: Kang *et al.* (2017) *Nat Biotechnol* 36: 89–94, [doi:10.1038/nbt.4042](https://doi.org/10.1038/nbt.4042) — fetched via Bioconductor's [`muscData::Kang18_8vs8()`](https://bioconductor.org/packages/muscData/) (same data the legacy `SeuratData::LoadData("ifnb")` route returned)
