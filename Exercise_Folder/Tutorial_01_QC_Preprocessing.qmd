...
dim_summary <- tibble::tribble(
  ...
  "Step 5 — subset() QC filter (nFeature_RNA, percent.mt)",  dim_after_filter[1],    dim_after_filter[2],
  "Step 6 — NormalizeData (no dimension change)",            dim_after_normalize[1], dim_after_normalize[2]
)
dim_summary
```
...
| Step 5 — `subset()` QC filter (`nFeature_RNA`, `percent.mt`)    | ~14,053 | ~13,500 |
| Step 6 — `NormalizeData` (no dimension change)                  | ~14,053 | ~13,500 |
