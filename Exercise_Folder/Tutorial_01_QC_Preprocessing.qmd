...
**`library(muscData)` errors with "there is no package called 'muscData'"** — it should already be installed from the [Tutorial 00 setup](Tutorial_00_Setup_RStudio_Packages.html); if it's missing, re-run that setup block.
...
**SoupX errors with "no raw matrix found"** (or `subscript out of bounds` from `load10X`) — this is correct behaviour. The `Kang18_8vs8()` download only ships the *filtered* counts. SoupX needs both. The Step 5 block is an **illustrative template** for your own 10x data and now stops with a clear message if you run it with the placeholder path. Run it for real on data that has a Cell Ranger `raw_feature_bc_matrix/` folder.
...
**Feature/gene-name message `replacing '_' with '-'`** — a few gene symbols in `ifnb` contain underscores, which Seurat disallows in feature names, so they're renamed to dashes (valid names like `HLA-A`). The loader does this explicitly and reports which genes; no marker used downstream has an underscore, so nothing breaks.
:::
...
::: {.callout-important title="Set up your project folder first — read this once"}
...
2. **Can't find a file?** The setup chunk prints the exact absolute path (`This module writes its figures & tables to: …`). If it's not where you expect, check `getwd()` — it should be your `scripts/` folder. See the downloadable **README** (linked on the [Materials](../Materials.html) page) for the full layout.
:::
...
::: callout-note
`muscData` (installed in the [Tutorial 00 setup](Tutorial_00_Setup_RStudio_Packages.html)) ships the data through Bioconductor's `ExperimentHub`: the first call to `Kang18_8vs8()` downloads \~25 MB into your local cache, and subsequent calls are instant.
:::
...
# A few gene (feature) symbols in this dataset contain underscores, and Seurat
# does not allow '_' in feature names: CreateSeuratObject() would silently rewrite
# them to '-' and print
#   "Feature names cannot have underscores ('_'), replacing with dashes ('-')".
# Dashes ARE valid Seurat gene names (e.g. HLA-A, MT-CO1), so the rewrite is
# harmless — but we do it explicitly here so the change is visible and
# reproducible, and we guard against the rare case where renaming would collide
# with an existing symbol (make.unique() de-duplicates if so).
cm <- counts(sce)
flagged <- grep("_", rownames(cm), value = TRUE)
if (length(flagged) > 0) {
  message(length(flagged), " feature name(s) contain '_'; renaming '_' -> '-': ",
          paste(head(flagged, 10), collapse = ", "),
          if (length(flagged) > 10) ", ..." else "")
...
-   Dataset: Kang *et al.* (2017) *Nat Biotechnol* 36: 89–94, [doi:10.1038/nbt.4042](https://doi.org/10.1038/nbt.4042) — fetched via Bioconductor's [`muscData::Kang18_8vs8()`](https://bioconductor.org/packages/muscData/)
