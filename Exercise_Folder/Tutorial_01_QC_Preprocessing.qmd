...
::: callout-note
**Companion book chapter:** [Chapter 1 — Preprocessing](../Resources_Folder/Chapter_01_Preprocessing.html) — the long-form prose treatment of this tutorial's material, with cross-references to the prerequisite appendices. · **HPC version (Talapas):** [Talapas analysis pipeline — `01_qc_preprocessing.R`](Talapas/Tutorial_Talapas_03_Analysis_Pipeline.html)
:::
...
::: callout-tip
**Want to run this on the cluster, or scale up later?** The **[Talapas HPC track](Talapas/Tutorial_Talapas_03_Analysis_Pipeline.html)** runs this *same* `ifnb` workflow as R + SLURM scripts — see [Tutorial — SLURM Basics](Talapas/Tutorial_Talapas_01_SLURM_Basics.html). To go all the way from **raw FASTQs** on a larger real dataset, see the optional [Full Talapas run from raw FASTQs](../Additional_Resources.html) in Additional Resources.
:::
...
```{r}
#| label: qc-plots
#| fig-cap: "Per-sample QC distributions (violins) and the UMI–gene relationship (scatter) for the ifnb dataset."

# Violin panel: four QC metrics, split by sample, fully labelled
p_qc_vln <- VlnPlot(ifnb,
...
p_qc_scatter <- FeatureScatter(ifnb,
               feature1 = "nCount_RNA",
...
# --- Figures out -----------------------------------------------------------
ggsave(file.path(out_dir, "Mod1_C5_qc_violins.png"),  p_qc_vln,
       width = 12, height = 4, dpi = 300)
ggsave(file.path(out_dir, "Mod1_C5_qc_scatter.png"),  p_qc_scatter,
...
# --- Figure out ------------------------------------------------------------
ggsave(file.path(out_dir, "Mod1_C7_doublet_violins.png"), p_dbl,
...
```{r}
#| label: hvgs
#| fig-cap: "Mean–variance relationship across genes with the top-10 highly variable features labelled."
...
ggsave(file.path(out_dir, "Mod1_C10_variable_features.png"), p_hvg,
