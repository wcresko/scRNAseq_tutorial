...
::: callout-important
**This is a Talapas Advanced exercise, not a laptop tutorial.** `cellranger-atac` requires a \~30 GB ATAC reference, multi-GB FASTQs, and a multi-core machine with 64+ GB RAM. Do not try to run it on your laptop. Work through the laptop sequence (Tutorials 01–12) and the [RNA Cell Ranger tutorial](Tutorial_11_CellRanger_RawData.html) first, then come here when you have your own raw 10x scATAC FASTQs.
:::
...
-   How to feed the outputs into **[Tutorial 09 — scATAC-seq with Signac](Tutorial_15_scATACseq_Signac.html)**
...
If your downstream Signac analysis needs the **older hg19** annotation (because [Tutorial 09](Tutorial_15_scATACseq_Signac.html) uses `EnsDb.Hsapiens.v75`), use the corresponding hg19 reference:
...
Then [Tutorial 09](Tutorial_15_scATACseq_Signac.html) loads them with `Read10X_h5()` + `CreateChromatinAssay()`.
...
-   [Talapas Advanced — Raw RNA Cell Ranger](Tutorial_11_CellRanger_RawData.html) — the RNA companion
-   [Tutorial 09 — scATAC-seq with Signac](Tutorial_15_scATACseq_Signac.html) — the downstream R analysis
