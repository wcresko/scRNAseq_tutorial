...
::: callout-important
**This is a Talapas Advanced exercise, not a laptop tutorial.** `cellranger-atac` requires a \~30 GB ATAC reference, multi-GB FASTQs, and a multi-core machine with 64+ GB RAM. Do not try to run it on your laptop. Work through the laptop sequence (Tutorials 01–12) and the [RNA Cell Ranger tutorial](Tutorial_Talapas_RawData_RNA.html) first, then come here when you have your own raw 10x scATAC FASTQs.
:::

## About this tutorial

The scATAC-seq counterpart to the [RNA Cell Ranger tutorial](Tutorial_Talapas_RawData_RNA.html). The 10x scATAC pipeline is conceptually parallel to Cell Ranger for RNA — same SLURM patterns, same FASTQ-naming rules, same reference-genome model — but the **outputs are different** (fragments, peaks, gene-activity scores) because chromatin accessibility is a different measurement than transcription.
...
-   How to feed the outputs into **[Tutorial 10 — scATAC-seq with Signac](../Tutorial_10_scATACseq_Signac.html)**
...
If your downstream Signac analysis needs the **older hg19** annotation (because [Tutorial 10](../Tutorial_10_scATACseq_Signac.html) uses `EnsDb.Hsapiens.v75`), use the corresponding hg19 reference:

```bash
# For matching Tutorial 10's hg19 / EnsDb.Hsapiens.v75:
...
## Step 6 — Hand off to Signac (Tutorial 10)

The four files Tutorial 10 expects are exactly what `cellranger-atac` produces. Copy them to your local `data/` directory:
...
Then [Tutorial 10](../Tutorial_10_scATACseq_Signac.html) loads them with `Read10X_h5()` + `CreateChromatinAssay()`.
...
-   [scNotebooks Module 11](https://integrativebioinformatics.github.io/scNotebooks/modules/en/module11/module11.html) — ArchR scATAC analysis (alternative to our Signac-based Tutorial 10)
-   [Talapas Advanced — Raw RNA Cell Ranger](Tutorial_Talapas_RawData_RNA.html) — the RNA companion
-   [Tutorial 10 — scATAC-seq with Signac](../Tutorial_10_scATACseq_Signac.html) — the downstream R analysis
