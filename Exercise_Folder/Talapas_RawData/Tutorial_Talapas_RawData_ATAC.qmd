---
title: "Talapas Advanced — Raw scATAC-seq Processing with cellranger-atac"
subtitle: "BCL → FASTQ → fragments + peaks on a SLURM cluster"
...
::: callout-important
**This is a Talapas Advanced exercise, not a laptop tutorial.** `cellranger-atac` requires a \~30 GB ATAC reference, multi-GB FASTQs, and a multi-core machine with 64+ GB RAM. Do not try to run it on your laptop. Work through the laptop sequence (Tutorials 01–12) and the [RNA Cell Ranger tutorial](../Talapas/Tutorial_Talapas_02_RawData_Cellranger.html) first, then come here when you have your own raw 10x scATAC FASTQs.
:::

## About this tutorial

The scATAC-seq counterpart to the [RNA Cell Ranger tutorial](../Talapas/Tutorial_Talapas_02_RawData_Cellranger.html). The 10x scATAC pipeline is conceptually parallel to Cell Ranger for RNA — same SLURM patterns, same FASTQ-naming rules, same reference-genome model — but the **outputs are different** (fragments, peaks, gene-activity scores) because chromatin accessibility is a different measurement than transcription.
...
If your downstream Signac analysis needs the **older hg19** annotation (because [Tutorial 09](../Tutorial_09_scATACseq_Signac.html) uses `EnsDb.Hsapiens.v75`), use the corresponding hg19 reference:
...
## Step 6 — Hand off to Signac (Tutorial 09)

The four files Tutorial 09 expects are exactly what `cellranger-atac` produces. Copy them to your local `data/` directory:
...
Then [Tutorial 09](../Tutorial_09_scATACseq_Signac.html) loads them with `Read10X_h5()` + `CreateChromatinAssay()`.
