---
title: "Tutorial 10 — WGCNA on COVID-19 PBMCs (GSE152418)"
...
::: callout-note
**Companion book chapter:** [Chapter 10 — WGCNA](../Resources_Folder/Chapter_10_WGCNA.html) — the long-form prose treatment of this tutorial's material, with cross-references to the prerequisite appendices.
:::
...
1.  Download the `.qmd` source: <a href="Tutorial_10_WGCNA.qmd.txt" download="Tutorial_10_WGCNA.qmd">Tutorial_10_WGCNA.qmd</a>. If your browser saves the file as `Tutorial_10_WGCNA.qmd.txt`, **drop the trailing `.txt`** so the filename ends in `.qmd`, then open it in RStudio.
2.  Download the GEO data (block below) into `../data/`.
3.  Work through the chunks, flipping `eval: true` as you go.
:::

::: callout-note
**Companion lecture:** [Lecture 10 — WGCNA](../Lecture_Folder/Lecture_10_WGCNA.html)
:::
...
```{r}
#| label: tutorial-10-wgcna-1
#| eval: false
library(GEOquery)

# ---------------------------------------------------------------------------
# Output directory for this module's figures and tables.
# Every figure/table chunk below writes a file named  Mod10_C<chunk>_<name>
# into ../output/Mod10/ so it can be cross-referenced from the rest of the site.
#   Mod10 = Module 10 (this tutorial);  C<n> = the nth code chunk.
# ---------------------------------------------------------------------------
out_dir <- "../output/Mod10"
...
phenoData |>
  ...
  readr::write_csv(file.path(out_dir, "Mod10_C3_pheno_data.csv"))
```
...
tibble::tibble(
  ...
  readr::write_csv(file.path(out_dir, "Mod10_C4_counts_dimensions.csv"))
```
...
tibble::tibble(
  ...
  readr::write_csv(file.path(out_dir, "Mod10_C5_good_samples_genes.csv"))
...
```{r}
#| label: hclust
#| fig-cap: "Average-linkage hierarchical clustering of samples (Euclidean distance) for outlier detection."
...
```{r}
#| label: pca-outlier
#| fig-cap: "Sample-level PCA (PC1 vs PC2) with each sample labelled, used to spot outlying samples."
pca       <- prcomp(t(data))
pca.dat   <- pca$x
pca.var   <- pca$sdev^2
...
# --- Figure out ------------------------------------------------------------
ggsave(file.path(out_dir, "Mod10_C7_sample_pca.png"), p_pca,
...
```{r}
#| label: pick-power
#| fig-cap: "Soft-threshold scan: scale-free topology fit (signed R²) and mean connectivity versus candidate powers."
...
ggsave(file.path(out_dir, "Mod10_C10_soft_threshold.png"), p_sft,
...
```{r}
#| label: dendro
#| fig-cap: "Gene dendrogram with unmerged and merged module-colour assignments from blockwiseModules."
...
module_eigengenes |>
  ...
  readr::write_csv(file.path(out_dir, "Mod10_C13_module_eigengenes.csv"))
...
tibble::enframe(table(bwnet$colors),
  ...
  readr::write_csv(file.path(out_dir, "Mod10_C13_genes_per_module.csv"))
```
...
```{r}
#| label: cor-trait
#| fig-cap: "Module–trait relationships: correlation of each module eigengene with the severity traits."
...
as.data.frame(module.trait.corr) |>
  ...
  readr::write_csv(file.path(out_dir, "Mod10_C15_module_trait_corr.csv"))

as.data.frame(module.trait.corr.p) |>
  tibble::rownames_to_column("module") |>
  readr::write_csv(file.path(out_dir, "Mod10_C15_module_trait_pvalues.csv"))
```
...
module.gene.mapping |>
  rownames_to_column("gene") |>
  readr::write_csv(file.path(out_dir, "Mod10_C16_gene_module_assignments.csv"))

mm |>
  rownames_to_column("gene") |>
  arrange(desc(MEturquoise)) |>
  head(15) |>
  readr::write_csv(file.path(out_dir, "Mod10_C16_turquoise_hub_genes.csv"))
```
