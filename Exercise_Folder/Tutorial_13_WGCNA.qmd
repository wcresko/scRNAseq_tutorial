...
```{r}
#| label: lst-M13-hclust
#| lst-cap: "Average-linkage hierarchical clustering of samples (Euclidean distance) for outlier detection."
...
```{r}
#| label: lst-M13-pca_outlier
#| lst-cap: "Sample-level PCA (PC1 vs PC2) with each sample labelled, used to spot outlying samples."
...
The hierarchical-clustering tree (@lst-M13-hclust) and the PCA (@lst-M13-pca_outlier) both expose outliers. Which would you trust more on a small dataset?
...
```{r}
#| label: lst-M13-pick_power
#| lst-cap: "Soft-threshold scan: scale-free topology fit (signed R²) and mean connectivity versus candidate powers."
...
```{r}
#| label: lst-M13-dendro
#| lst-cap: "Gene dendrogram with unmerged and merged module-colour assignments from blockwiseModules."
...
```{r}
#| label: lst-M13-cor_trait
#| lst-cap: "Module–trait relationships: correlation of each module eigengene with the severity traits."
...
```{r}
#| label: lst-M13-kme
#| lst-cap: "List genes in a module of interest and rank them by kME (module membership correlation)."
