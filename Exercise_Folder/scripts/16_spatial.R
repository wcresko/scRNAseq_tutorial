...
# Figures/tables (match the Mod11 notebook filenames): ../output/Mod11/Mod11_C*_*
...
OUT_DIR <- Sys.getenv("OUT_DIR", "../output/Mod11") # figures/tables, named to match Tutorial_11.qmd
...
# Table out: per-section dimensions (features x spots) (Mod11_C2)
tibble(section  = c("anterior1", "posterior1"),
       features = c(nrow(ant), nrow(post)),
       spots    = c(ncol(ant), ncol(post))) |>
  write_csv(file.path(OUT_DIR, "Mod11_C2_section_dimensions.csv"))
...
# Figure out: per-spot QC violins for the anterior section (Mod11_C3)
...
p_qc_vln <- p_qc_vln + plot_annotation(
  ...
  caption  = "Module 11 · Spatial Transcriptomics")
ggsave(file.path(OUT_DIR, "Mod11_C3_qc_violins.png"), p_qc_vln, width = 10, height = 4, dpi = 300)

# Figure out: spatial QC overlay on the tissue (Mod11_C3)
...
p_qc_spatial <- p_qc_spatial + plot_annotation(
  ...
  caption  = "Module 11 · Spatial Transcriptomics")
ggsave(file.path(OUT_DIR, "Mod11_C3_qc_spatial.png"), p_qc_spatial, width = 10, height = 5, dpi = 300)
...
# Step 4 — Figure out: known anatomical marker genes on the tissue (Mod11_C5)
p_markers_ant <- SpatialFeaturePlot(ant, features = c("Hpca", "Plp1", "Sst"),
  ...
  plot_annotation(
    ...
    caption  = "Module 11 · Spatial Transcriptomics")
ggsave(file.path(OUT_DIR, "Mod11_C5_markers_anterior.png"), p_markers_ant, width = 12, height = 4, dpi = 300)
...
p_markers_post <- SpatialFeaturePlot(post, features = c("Hpca", "Plp1", "Pcp4"),
  ...
  plot_annotation(
    ...
    caption  = "Module 11 · Spatial Transcriptomics")
ggsave(file.path(OUT_DIR, "Mod11_C5_markers_posterior.png"), p_markers_post, width = 12, height = 4, dpi = 300)

# Step 5 — Cluster spots on the anterior section (notebook Mod11_C6); also needed so the
...
# Figure out: anterior spot clusters in UMAP space and on tissue (Mod11_C6)
...
p_cluster <- (p_umap + p_tis) + plot_annotation(
  ...
  caption  = "Module 11 · Spatial Transcriptomics")
ggsave(file.path(OUT_DIR, "Mod11_C6_anterior_clusters.png"), p_cluster, width = 12, height = 5, dpi = 300)
...
# Figure + table out: top spatially variable genes (Mod11_C7)
top_sv <- SpatiallyVariableFeatures(ant, selection.method = "moransi")[1:6]
tibble(rank = seq_along(top_sv), gene = top_sv) |>
  write_csv(file.path(OUT_DIR, "Mod11_C7_spatially_variable_features.csv"))
p_svf <- SpatialFeaturePlot(ant, features = top_sv, ncol = 3, alpha = c(0.1, 1)) +
  plot_annotation(
    ...
    caption  = "Module 11 · Spatial Transcriptomics")
ggsave(file.path(OUT_DIR, "Mod11_C7_spatially_variable_features.png"), p_svf, width = 12, height = 7, dpi = 300)
...
# Figure out: integrated UMAP coloured by section and joint cluster (Mod11_C8)
p_int_umap <- DimPlot(brain, group.by = c("slice", "seurat_clusters")) +
  plot_annotation(
    ...
    caption  = "Module 11 · Spatial Transcriptomics")
ggsave(file.path(OUT_DIR, "Mod11_C8_integrated_umap.png"), p_int_umap, width = 12, height = 5, dpi = 300)

# Figure out: joint clusters mapped back onto each section (Mod11_C8)
p_int_spatial <- SpatialDimPlot(brain, label = TRUE) +
  plot_annotation(
    ...
    caption  = "Module 11 · Spatial Transcriptomics")
ggsave(file.path(OUT_DIR, "Mod11_C8_integrated_spatial_clusters.png"), p_int_spatial, width = 10, height = 5, dpi = 300)
...
cat("Wrote Mod11 figures/tables to", OUT_DIR, "\n")
