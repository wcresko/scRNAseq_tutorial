...
OUT_DIR  <- Sys.getenv("OUT_DIR",  "../output/Mod10") # figures/tables, named to match Tutorial_10.qmd
...
# Table out: the trimmed sample metadata (phenoData) (Mod10_C3)
phenoData |>
  rownames_to_column("geo_accession_rowname") |>
  write_csv(file.path(OUT_DIR, "Mod10_C3_pheno_data.csv"))
...
# Table out: counts-matrix dimensions (genes x samples) (Mod10_C4)
tibble(stage   = "Step 1 — tidy counts matrix (Ensembl x GEO accession)",
       genes   = nrow(data),
       samples = ncol(data)) |>
  write_csv(file.path(OUT_DIR, "Mod10_C4_counts_dimensions.csv"))

# Step 2 — Outlier detection; drop bad genes + obvious outlier samples
gsg  <- goodSamplesGenes(t(data))

# Table out: goodSamplesGenes outcome (genes/samples flagged) (Mod10_C5)
tibble(all_ok         = gsg$allOK,
  ...
  write_csv(file.path(OUT_DIR, "Mod10_C5_good_samples_genes.csv"))

data <- data[gsg$goodGenes == TRUE, ]

# Figure out (base graphics): sample dendrogram for outlier detection (Mod10_C6)
htree <- hclust(dist(t(data)), method = "average")
png(file.path(OUT_DIR, "Mod10_C6_sample_dendrogram.png"),
...
# Figure out: sample-level PCA (PC1 vs PC2) for outlier detection (Mod10_C7)
pca       <- prcomp(t(data))
pca.dat   <- pca$x
pca.var   <- pca$sdev^2
...
p_pca <- ggplot(as.data.frame(pca.dat), aes(PC1, PC2)) +
  ...
  labs(title    = "Sample PCA for outlier detection — GSE152418 PBMCs",
       ...
       caption  = "Module 10 · WGCNA") +
  theme_minimal()
ggsave(file.path(OUT_DIR, "Mod10_C7_sample_pca.png"), p_pca, width = 7, height = 6, dpi = 300)
...
# Figure out: soft-threshold scan — scale-free fit + mean connectivity (Mod10_C10)
...
p_sft <- (a1 / a2) +
  plot_annotation(
    ...
    caption  = "Module 10 · WGCNA")
ggsave(file.path(OUT_DIR, "Mod10_C10_soft_threshold.png"), p_sft, width = 7, height = 8, dpi = 300)
...
# Figure out (base graphics): gene dendrogram + module colours (Mod10_C12)
png(file.path(OUT_DIR, "Mod10_C12_module_dendrogram.png"),
    width = 10, height = 6, units = "in", res = 300)
...
# Tables out: module eigengenes and genes-per-module (Mod10_C13)
module_eigengenes |>
  rownames_to_column("sample") |>
  write_csv(file.path(OUT_DIR, "Mod10_C13_module_eigengenes.csv"))
enframe(table(bwnet$colors), name = "module", value = "n_genes") |>
  mutate(n_genes = as.integer(n_genes)) |>
  arrange(desc(n_genes)) |>
  write_csv(file.path(OUT_DIR, "Mod10_C13_genes_per_module.csv"))
...
# Figure out (base graphics): module-trait correlation heatmap (Mod10_C15)
heatmap.data <- merge(module_eigengenes, traits, by = "row.names")
heatmap.data <- column_to_rownames(heatmap.data, var = "Row.names")
png(file.path(OUT_DIR, "Mod10_C15_module_trait_heatmap.png"),
...
# Tables out: module-trait correlations + p-values (Mod10_C15)
as.data.frame(module.trait.corr) |>
  rownames_to_column("module") |>
  write_csv(file.path(OUT_DIR, "Mod10_C15_module_trait_corr.csv"))
as.data.frame(module.trait.corr.p) |>
  rownames_to_column("module") |>
  write_csv(file.path(OUT_DIR, "Mod10_C15_module_trait_pvalues.csv"))
...
# Tables out: gene-module assignments + top turquoise kME hub genes (Mod10_C16)
module.gene.mapping |>
  rownames_to_column("gene") |>
  write_csv(file.path(OUT_DIR, "Mod10_C16_gene_module_assignments.csv"))
mm |>
  rownames_to_column("gene") |>
  arrange(desc(MEturquoise)) |>
  head(15) |>
  write_csv(file.path(OUT_DIR, "Mod10_C16_turquoise_hub_genes.csv"))

cat("Wrote WGCNA soft-power, module assignments, and trait-correlation tables to", OBJ_DIR, "\n")
cat("Wrote Mod10 figures/tables to", OUT_DIR, "\n")
