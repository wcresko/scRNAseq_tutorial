...
::: callout-note
**Companion book chapter:** [Chapter 3 — Markers & Manual Annotation](../Resources_Folder/Chapter_03_Markers_Annotation.html) — the long-form prose treatment of this tutorial's material, with cross-references to the prerequisite appendices.
:::
...
-   Saving the annotated object so Tutorial 04 (Integration) can pick it up

::: callout-note
We're annotating the **unintegrated** object. Why? Because **markers are cell-type-driven, not batch-driven** — even when an embedding is dominated by a batch effect, the genes that distinguish T cells from B cells are still T-cell genes. Working markers up before integration is a useful sanity check: if a marker gene cleanly identifies a cluster on the unintegrated data, it should still cleanly identify the *biologically equivalent* cluster after integration in Tutorial 04.
:::
...
**Markers list is dominated by ISGs (`ISG15`, `IFI6`, `IFIT3`, `MX1`)** — that's expected on the *unintegrated* `ifnb` data because clusters here are partly driven by `stim`. Tutorial 04 (Integration) fixes this; cell-type-specific markers reclaim the top of the lists after integration.
...
1.  Those are **condition markers** — they distinguish IFN-β–stimulated cells from control cells, *not* cell types. They top the list because cluster identities at this stage are partly driven by `stim`. Tutorial 04 will integrate the data so cluster identities track biology, not condition.
2.  No. `FindAllMarkers` / `FindMarkers` treat each cell as an independent observation, which is **anti-conservative** for condition-level comparisons across biological samples. For STIM vs CTRL within a cell type, use **pseudobulk** + DESeq2/edgeR/limma — that's exactly what Tutorial 05 covers, on this dataset.
...
DoHeatmap(ifnb, features = top10$gene) + NoLegend()
```
...
DotPlot(ifnb, features = canonical_markers) + RotatedAxis()
```
...
p_manual <- DimPlot(ifnb, reduction = "umap", label = TRUE, repel = TRUE) +
  NoLegend() + ggtitle("Manual annotation (yours)")
p_truth  <- DimPlot(ifnb, reduction = "umap", group.by = "seurat_annotations",
                    label = TRUE, repel = TRUE) +
  NoLegend() + ggtitle("Author labels (ground truth)")

p_manual
p_truth

# Cross-tabulate. Each row should ideally concentrate on a single column.
# table(manual = Idents(ifnb), truth = ifnb$seurat_annotations)
```
...
3.  In Tutorial 04, after integration, the same cluster numbers will mean different things. Why is `seurat_annotations` (a per-cell label) more durable than your `RNA_snn_res.0.5` manual labels (a per-cluster mapping)?
...
In **[Tutorial 04 — Two-Sample Integration](Tutorial_04_Integration.html)** we'll re-do the PCA + neighbor-graph + clustering steps on an **integrated** representation that aligns CTRL and STIM, and watch what happens to:
...
You'll come out of it with a clean `.rds` ready for **pseudobulk DE** in Tutorial 05.
