---
title: "Tutorial 09 — scATAC-seq with Signac"
subtitle: "From peaks matrix to clustered cells (PBMC 10k v1, 10x — same dataset as Patel's `scATACSeq_standard_workflow.R`)"
...
A parallel to the scRNA-seq tutorial series — but for **chromatin accessibility** (scATAC-seq). The workflow follows the standard **Signac** pipeline from the Stuart Lab on the 10x PBMC 10k scATAC-seq dataset, **the same dataset Patel uses in [`scATACSeq_standard_workflow.R`](https://github.com/kpatel427/YouTubeTutorials/blob/main/scATACSeq_standard_workflow.R)**.

-   Reference vignette: [Signac PBMC scATAC-seq vignette](https://stuartlab.org/signac/articles/pbmc_vignette)
-   Companion lecture: [Lecture 09 — Bulk + scATAC-seq](../Lecture_Folder/Lecture_15_ATACseq.html)
-   Alternative toolkit walkthrough: [scNotebooks Module 11](https://integrativebioinformatics.github.io/scNotebooks/modules/en/module11/module11.html) does the same kind of analysis with **ArchR** (peak calling, motif enrichment, TF regulators, scATAC↔scRNA integration, trajectory) on Kumegawa *et al.* 2022 breast-cancer data — a useful cross-toolkit reference

::: {.callout-tip title="How to use this page"}
The rendered HTML shows the code but does **not** execute it. To run it:

1.  Download the `.qmd` source: <a href="Tutorial_15_scATACseq_Signac.qmd.txt" download="Tutorial_15_scATACseq_Signac.qmd">Tutorial_15_scATACseq_Signac.qmd</a>. If your browser saves the file as `Tutorial_15_scATACseq_Signac.qmd.txt`, **drop the trailing `.txt`** so the filename ends in `.qmd`, then open it in RStudio.
...
::: callout-note
**Companion book chapter:** [Chapter 09 — scATAC-seq](../Resources_Folder/Chapter_15_ATAC.html) — the long-form prose treatment of this tutorial's material, with cross-references to the prerequisite appendices.
:::


## Dataset — PBMC 10k scATAC-seq v1 (10x Genomics, hg19)

**What it is.** Approximately ten-thousand human **peripheral blood mononuclear cells (PBMCs)** from a single healthy donor, run on the 10x Chromium **Single Cell ATAC v1** assay. This is the canonical tutorial dataset used across Signac, ArchR, and EpiScanpy vignettes — and is the dataset referenced in Patel's tutorial.

-   Landing page: [10x Genomics — PBMC 10k scATAC-seq v1 (hg19)](https://www.10xgenomics.com/datasets/10-k-peripheral-blood-mononuclear-cells-pbm-cs-from-a-healthy-donor-1-standard-1-1-0)
-   Reference genome: **hg19 / GRCh37** (hence `EnsDb.Hsapiens.v75` below)
-   Chemistry: 10x Chromium Single Cell ATAC v1
-   Cell count: \~8,700 post-filter cells

::: callout-warning
Only the **v1** release is pinned to hg19. The *Next GEM v1.1* re-release of the same sample is mapped to **hg38** and uses file names prefixed with `atac_v1_pbmc_10k_nextgem_*`. This exercise — like Patel's tutorial — assumes the original hg19 release.
:::

### Download with `wget` or `curl`

Direct CDN links for the v1 hg19 release. Place all four files into `../data/` (relative to the tutorial).
...
::: {.callout-tip title="When you need to liftOver — hg19 ↔ hg38"}
...
# Every figure/table chunk below writes a file named  Mod9_C<chunk>_<name>
# into ../output/Mod9/ so it can be cross-referenced from the rest of the site.
#   Mod9 = Module 9 (this tutorial);  C<n> = the nth code chunk.
# ---------------------------------------------------------------------------
out_dir <- "../output/Mod9"
...
You will need three files from the 10x PBMC 10k scATAC-seq release, placed under `../data/`. All three are available under "Output and supplemental files" on the [10x landing page](https://www.10xgenomics.com/datasets/10-k-peripheral-blood-mononuclear-cells-pbm-cs-from-a-healthy-donor-next-gem-v-1-1-1-1-standard-2-0-0).
...
| # | Column | Description |
|---|---|---|
| 1 | `chrom` | Chromosome (`chr1` … `chrY`, plus contigs). |
...
readr::write_csv(tibble::as_tibble(frag_preview),
                 file.path(out_dir, "Mod9_C2_fragment_preview.csv"))
```
...
```{r}
#| label: qc-viz
#| fig-cap: "ATAC QC density scatters (depth/nucleosome signal vs TSS enrichment) and per-metric violins for the PBMC 10k cells."
...
```{r}
#| label: lsi
#| fig-cap: "Correlation between each LSI component and sequencing depth; component 1 is the depth-confounded one to drop."
...
```{r}
#| label: umap-cluster
#| fig-cap: "UMAP of the PBMC 10k scATAC-seq cells coloured by SLM cluster (LSI dims 2:30)."
...
::: callout-warning
**HOMER's `-size` flag matters.** `-size 200` centres a 200 bp window on each peak — appropriate for ATAC-seq peaks (typically 500–1000 bp wide). For wider regions or full-peak coverage use `-size given`. See the [HOMER peakMotifs notes](http://homer.ucsd.edu/homer/ngs/peakMotifs.html).
:::
...
-   Reference vignette: [Signac PBMC scATAC-seq vignette](https://stuartlab.org/signac/articles/pbmc_vignette)
-   Workflow source: Khushbu Patel, single-cell ATAC-seq standard workflow ([scATAC-seq](https://github.com/kpatel427/YouTubeTutorials/blob/main/scATACSeq_standard_workflow.R), [HOMER motif workflow](https://github.com/kpatel427/YouTubeTutorials/blob/main/runHomer.sh), [liftOver](https://github.com/kpatel427/YouTubeTutorials/blob/main/liftcoordinates.R))
