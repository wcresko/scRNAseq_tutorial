...
SpatialFeaturePlot(ant, features = c("Hpca", "Plp1", "Sst"),
                   ncol = 3, alpha = c(0.1, 1))
SpatialFeaturePlot(post, features = c("Hpca", "Plp1", "Pcp4"),
                   ncol = 3, alpha = c(0.1, 1))
```
...
p_umap <- DimPlot(ant, reduction = "umap", label = TRUE) +
            ggtitle("Anterior — UMAP")
p_tis  <- SpatialDimPlot(ant, label = TRUE, label.size = 3) +
            ggtitle("Anterior — clusters on tissue")
...
SpatialFeaturePlot(ant, features = top_sv, ncol = 3, alpha = c(0.1, 1))
```
...
# Joint UMAP
DimPlot(brain, group.by = c("slice", "seurat_clusters"))
