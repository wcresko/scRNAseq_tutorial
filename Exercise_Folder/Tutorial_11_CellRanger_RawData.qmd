---
title: "Tutorial 11 — Raw Data Download + Cell Ranger `count` (bonus, upstream)"
subtitle: "BCL → FASTQ → counts on a SLURM cluster — the raw-data step that feeds the core pipeline"
...
::: callout-note
**Companion lecture:** [Lecture 11 — Cell Ranger: Raw Reads to Counts](../Lecture_Folder/Lecture_11_CellRanger.html) · **Companion reading:** [Chapter 11 — Cell Ranger: From Raw Reads to a Counts Matrix](../Resources_Folder/Chapter_11_CellRanger.html)
:::
...
-   A Talapas account ([request one](https://racs.uoregon.edu/request-access)) and basic familiarity with SLURM (see the [SLURM Basics tutorial](Tutorial_09_SLURM_Basics.html))
...
```{bash}
#| label: M11-projects_directory_not_home
#| eval: false
...
```{bash}
#| label: M11-human_grch38_current_release
#| eval: false
...
```{bash}
#| label: M11-web_summary_html_open
#| eval: false
...
```{r}
#| label: M11-pick_up_core_pipeline
#| eval: false
library(Seurat)
...
# … pick up at the core pipeline (Module 10), script 01 (QC metrics)
```
...
-   [Tutorial 12 — Raw scATAC-seq with cellranger-atac](Tutorial_12_scATAC_CellRanger_RawData.html) — the ATAC-seq companion
-   [Tutorial SLURM Basics](Tutorial_09_SLURM_Basics.html) — SLURM fundamentals on Talapas
-   [10x Genomics Cell Ranger documentation](https://www.10xgenomics.com/support/software/cell-ranger/latest)
-   [Tutorial 10 — The Analysis Pipeline](Tutorial_10_Talapas_Pipeline.html) — the R-script pipeline that picks up downstream from the Cell Ranger output
