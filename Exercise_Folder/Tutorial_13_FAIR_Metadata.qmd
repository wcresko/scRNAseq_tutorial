...
The hands-on companion to **[Lecture 13 — FAIR Principles & Data Sharing](../Lecture_Folder/Lecture_13_FAIR_DataSharing.html)** and the parallel of [scNotebooks Module 13](https://integrativebioinformatics.github.io/scNotebooks/modules/en/module13/module13.html). We take the annotated `ifnb` object from Tutorial 04 and walk through what it would take to **deposit it for real** in NCBI + CELLxGENE Discover:


::: callout-note
**Companion book chapter:** [Chapter 13 — FAIR & Data Sharing](../Resources_Folder/Chapter_13_FAIR.html) — the long-form prose treatment of this tutorial's material, with cross-references to the prerequisite appendices.
:::
...
|---|---|---|
| `../data/ifnb_annotated_final.rds` | Tutorial 04 | Seurat object with PCA/UMAP, manual labels, SingleR labels, Azimuth labels, and the reconciled `celltype_final` per cell. |
...
**`cellxgene-schema validate` reports "ontology term not found"** — the ontology IDs (`CL:`, `UBERON:`, `MONDO:`) get updated periodically. The validator pulls fresh ontology versions; an ID that worked a year ago may be deprecated. Look up the current ID at <https://www.ebi.ac.uk/ols4/>.
...
# Every figure/table chunk below writes a file named  Mod13_C<chunk>_<name>
# into ../output/Mod13/ so it can be cross-referenced from the rest of the site.
...
tibble::tibble(metadata_column = colnames(ifnb@meta.data)) |>
  readr::write_csv(file.path(out_dir, "Mod13_C2_metadata_columns.csv"))

tibble::enframe(table(ifnb$stim),
                name = "stim", value = "n_cells") |>
  dplyr::mutate(n_cells = as.integer(n_cells)) |>
  readr::write_csv(file.path(out_dir, "Mod13_C2_cells_per_sample.csv"))
```
...
readr::write_csv(canonical, file.path(out_dir, "Mod13_C3_canonical_sample_sheet.csv"))
```
...
readr::write_csv(per_cell, file.path(out_dir, "Mod13_C4_per_cell_metadata.csv"))
```
...
tibble::enframe(table(ifnb$cell_type_ontology_term_id),
  ...
  readr::write_csv(file.path(out_dir, "Mod13_C5_cells_per_ontology_term.csv"))
```
...
readr::write_csv(biosample, file.path(out_dir, "Mod13_C7_biosample_template.csv"))
```
...
readr::write_csv(sra, file.path(out_dir, "Mod13_C8_sra_template.csv"))
```
...
readr::write_csv(geo_samples, file.path(out_dir, "Mod13_C9_geo_samples_template.csv"))
```
...
-   [Lecture 13 — FAIR Principles & Data Sharing](../Lecture_Folder/Lecture_13_FAIR_DataSharing.html)
