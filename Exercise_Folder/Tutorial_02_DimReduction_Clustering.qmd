...
**`FindClusters` is slow** — `presto` (installed in the [Tutorial 00 setup](Tutorial_00_Setup_RStudio_Packages.html)) gives a much faster Wilcoxon implementation; Seurat detects it automatically.
...
```{r}
#| label: lst-M2-pca
#| lst-cap: "Run PCA on the 2,000 HVGs, print top gene loadings for PCs 1–5, and draw the PC1 loading heatmap."
...
```{r}
#| label: lst-M2-elbow
#| lst-cap: "ElbowPlot of per-PC standard deviation across 40 PCs — used to choose a defensible PC cutoff."
...
```{r}
#| label: lst-M2-cluster
#| lst-cap: "Run Louvain clustering at five resolutions and inspect the resulting metadata columns."
...
```{r}
#| label: lst-M2-umap
#| lst-cap: "Compute UMAP on the top 20 PCs and plot with Louvain clusters (resolution 0.5) as colors."
...
This is the key exercise. The UMAP in @lst-M2-umap is colored by cluster. Now color the *same UMAP* by **sample** and by **author cell-type label** and compare.

```{r}
#| label: lst-M2-batch_diagnostic
#| lst-cap: "Color the UMAP by sample (stim) and by author cell-type label to diagnose the batch effect."
...
```{r}
#| label: lst-M2-crosstab
#| lst-cap: "Cross-tabulate cluster membership by sample and by author cell-type label to measure how mixed and how pure the unintegrated clusters are."
...
-   Dataset: `ifnb` via Bioconductor's `muscData::Kang18_8vs8()` — Kang *et al.* (2017) *Nat Biotechnol* 36: 89–94
