...
subtitle: "PCA, how many PCs, neighbor graph, Leiden at multiple resolutions, UMAP — and seeing the batch effect"
...
1.  PC1 in the unintegrated `ifnb` object is heavily loaded with ISGs because the IFN-β response is the largest source of variance in this dataset. **PC1 is essentially `stim` (CTRL vs STIM)** — i.e. the batch / condition axis, not a cell-type axis. That's the batch effect we'll fix in Tutorial 04.
...
4.  Yes. The batch axis is dominating PC1 and is splitting biologically identical cell types across two clusters. **Tutorial 04** will integrate the data with Harmony and Seurat anchors, and you'll see the same cells re-arrange into shared, sample-mixed clusters.
...
::: callout-tip
Continue with **Tutorial 03 — Markers & Cell Type Annotation**. We'll do annotation on the unintegrated clusters first (it's still possible — markers are cell-type-driven even when the embedding is sample-driven), then in **Tutorial 04** we'll redo PCA + clustering on the integrated representation and watch the embedding reorganize.
:::
