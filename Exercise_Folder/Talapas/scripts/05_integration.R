...
# In:   ../objects/nsclc_annotated_final.rds   Out: ../objects/nsclc_integrated.rds
...
# NSCLC ships no per-cell donor IDs (CMO demultiplexing not run) -> synthesize 7
# pseudo-donors to demonstrate the batch-correction workflow (NOT real biology).
if (!"donor" %in% colnames(seu@meta.data))
  seu$donor <- paste0("d", sample(1:7, ncol(seu), replace = TRUE))

# Step 1 — re-PCA, then Harmony-correct over the donor variable
