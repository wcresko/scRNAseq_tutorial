---
title: "Talapas 01 — SLURM Basics on Talapas"
...
All examples are specific to UO and Talapas. If you have not yet logged in to Talapas for the first time, see the [**VS Code & Talapas**](../../Resources_Folder/VSCode_Talapas.html) reference first.
...
You don't have to type it out: the Talapas scripts folder includes [`01_slurm_demo.sbatch`](scripts/01_slurm_demo.sbatch) (the same idea). Open it in VS Code, set `--account`, and `sbatch 01_slurm_demo.sbatch`. Because it's a very short job, it may finish before you can catch it in `squeue`.
:::
...
Rscript scripts/03_qc_preprocess.R
```
...
sbatch --job-name=qc    run_rscript.sbatch 03_qc_preprocess.R
```
...
This is exactly how the rest of the Talapas Advanced track is run — every analysis step (Talapas 03–10) is an R script submitted through this wrapper.
